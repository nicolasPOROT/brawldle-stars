const Utils = {
  ...
  getCompletedModes() {
    ...
    return saved && saved.date === this.getToday()
      ? new Set(saved.modes)
      : new Set();
  },
  ...
  async getTodaysModes() {
    ...
    url.searchParams.set(
      "select",
      "mode_id,mode_slot,game_mode(id,name,is_enabled)",
    );
    ...
    document.querySelectorAll(".play-cta").forEach((link) => {
      link.addEventListener("click", async (event) => {
        ...
        try {
          ...
          const modes = (await this.getTodaysModes()).filter(
            (row) => row.game_mode?.is_enabled,
          );
          const next =
            modes.find((row) => !completed.has(String(row.mode_id))) ||
            modes[0];
